...
class Setup:
    ...
    def boost(self):
        ...
        if isMac():
          ...
          cmd = """echo "using gcc : 4.9 : /usr/bin/g++-4.9 ; " >> tools/build/v2/user-config.jam &&./bootstrap.sh --prefix={local_dir} && ./b2 -d 2 toolset=gcc-4.9 -j {num_cores} install""".format(local_dir=shellquote(i.local_dir).replace(' ', '\ '), num_cores=self.num_cores())
        self.__build(i, cmd)
    ...
    def Rdevel(self):
        ...
        if isMac():
          cmd = """
            ./configure --prefix={local_dir} --enable-R-shlib --with-x=no CC=gcc-4.9 CXX=g++-4.9
            && make -j {num_cores}
            && make install
