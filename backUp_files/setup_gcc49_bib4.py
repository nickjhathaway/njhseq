...
class Setup:
    ...
    def boost(self):
        ...
        if isMac():
          ...
          cmd = """echo "using gcc : 4.8 : /home/hathawan/bin/g++ ; " >> tools/build/v2/user-config.jam &&./bootstrap.sh --prefix={local_dir} && ./b2 -d 2 toolset=gcc-4.8 -j {num_cores} install""".format(local_dir=shellquote(i.local_dir).replace(' ', '\ '), num_cores=self.num_cores())
        self.__build(i, cmd)
    ...
    def Rdevel(self):
        ...
        if isMac():
          cmd = """
            ./configure --prefix={local_dir} --enable-R-shlib --with-x=no CC=gcc CXX=g++
            && make -j {num_cores}
            && make install
    ...
    def cppcms(self):
        ...
        if(sys.platform == "darwin"):
            cmd = "mkdir -p build && cd build && CC=gcc CXX=g++ cmake -DCMAKE_INSTALL_PREFIX:PATH={local_dir} .. && make -j {num_cores} install && install_name_tool -change libbooster.0.dylib {local_dir}/lib/libbooster.0.dylib {local_dir}/lib/libcppcms.1.dylib".format(local_dir=shellquote(i.local_dir), num_cores=self.num_cores())
        else:
            cmd = "mkdir -p build && cd build && CC=gcc CXX=g++ cmake -DCMAKE_INSTALL_PREFIX:PATH={local_dir} .. && make -j {num_cores} install ".format(local_dir=shellquote(i.local_dir), num_cores=self.num_cores())
...
mkdir -p build
&& cd build
&& CC=gcc-4.8 CXX=g++ cmake -D DEBUG=OFF -D PROFILE=OFF
