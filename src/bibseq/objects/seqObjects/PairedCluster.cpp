...
void PairedCluster::calculateConsensus(aligner & alignerObj, bool setToConsensus){
  ...
  std::function<const seqInfo& (const PairedRead& )> getSeqBase =
			[](const PairedRead& read) ->const seqInfo& {return read.seqBase_;};
	std::function<const seqInfo& (const PairedRead& )> getSeqBaseMate =
			[](const PairedRead & read) ->const seqInfo& {return read.mateSeqBase_;};
			...
			if(sub.second.size() > 1){
				...
				if(qualRep == "max"){
					...
					for(const auto & qualPos : iter::range(clusters[sub.second.front()].seqBase_.qual_.size())){
						...
						clusters[sub.second.front()].seqBase_.qual_[qualPos] = vectorMedian(quals);
					}
				}else {
...
void PairedCluster::writeOutClusters(const std::string& directoryName,const readObjectIOOptions & ioOptions) const{
	std::ofstream outFile1;
	openTextFile(outFile1, directoryName + seqBase_.name_ + "_R1", ioOptions.outExtention_, ioOptions);
	std::ofstream outFile2;
	openTextFile(outFile2, directoryName + seqBase_.name_ + "_R2", ioOptions.outExtention_, ioOptions);
	for(const auto & read : reads_){
		read.seqBase_.outPut(outFile1, ioOptions);
		read.mateSeqBase_.outPut(outFile2, ioOptions);
	}
}
