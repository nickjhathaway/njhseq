...
void debGraph::travelGraphEulerianCycleStyle() {
  ...
  while (keepGrowing) {
    ...
    _path = catenateVectors(getSubVector(_path, pathPos, len(_path) - pathPos),
...
void debGraph::travelGraphEulerianPathStyle() {
  ...
  while (keepGrowing) {
    ...
    _path.pop_back();
    uint32_t nextNode = _gen.unifRand<uint32_t>(0, len(realExplorableNodes));
    // std::cout<<"current path "<<std::endl;
    /*for(const auto & p : _path){
     std::cout<<_nodes[p]._value<<"->";
     }
  ...
  // rearrange path
  _path =
      catenateVectors(getSubVector(_path, theEnd + 1, len(_path) - 2 - theEnd),
