    ...
    for (const auto& n : nodes_) {
      ...
      for (const auto& secondN : nodes_) {
        if (n.read_.seqBase_.name_ != secondN.read_.seqBase_.name_ &&
          ...
          if (alignerObj.distances_.percentIdentity_ > bestIdentity) {
            bestIdentity = alignerObj.distances_.percentIdentity_;
            distance = alignerObj.distances_.ownDistance_;
            gapDistance = alignerObj.distances_.ownGapDistance_;
            misMatches = alignerObj.errors_.hqMismatches_;
            bestPos = pos;
            foundParent = true;
            if (alignerObj.distances_.percentageGaps_ > 0) {
              differsByIndel = true;
            } else {
              differsByIndel = false;
            }
            if (alignerObj.distances_.percentIdentity_ < 1.0) {
