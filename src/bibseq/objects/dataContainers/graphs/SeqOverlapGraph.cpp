...
SeqOverlapGraph::node::node(const std::shared_ptr<seqInfo> & val) :
	...
	void SeqOverlapGraph::node::addToPath(std::ostream & out, std::string currentPath){
		++visitCount_;
	...
	SeqOverlapGraph::edge::edge(const std::shared_ptr<node> & head, const std::shared_ptr<node> & tail) :
...
void SeqOverlapGraph::addNode(const std::shared_ptr<seqInfo> & n){
	if(bib::has(nodes_, n->name_)){
		std::stringstream ss;
		ss << __PRETTY_FUNCTION__ << ": Error, alreay contains node with name " << n->name_ << "\n";
		throw std::runtime_error{ss.str()};
	}
	nodes_.emplace(n->name_, std::make_shared<node>(n));
}

void SeqOverlapGraph::addEdge(const std::string & head, const std::string & tail){
...
void SeqOverlapGraph::writePaths(std::ostream & out)const{
	...
	if(!notVisitedNodes.empty()){
		std::stringstream ss;
		ss << __PRETTY_FUNCTION__ << ": Error, the following nodes weren't visited:\n";
