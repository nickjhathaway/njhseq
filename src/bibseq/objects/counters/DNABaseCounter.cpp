...
Json::Value DNABaseCounter::toJson() const{
	...
	ret["gcContent"] = bib::json::toJson(calcGcContent());
...
double DNABaseCounter::calcGcContent()const{
	return (bases_['G'] + bases_['C'] + bases_['g'] + bases_['c'])/getTotalCount();
}
...
std::string DNABaseCounter::outPutInfoHeader(){
	return bib::conToStr(toVecStr("base", "count", "fraction"), "\t");
}
// output data
void DNABaseCounter::outPutInfo(std::ostream &out, bool header) const{
	...
	for(auto c : alphabet_ ){
		out << bib::conToStr(toVecStr(c, bases_[c], baseFracs_[c]), "\t") << std::endl;
	}
}
