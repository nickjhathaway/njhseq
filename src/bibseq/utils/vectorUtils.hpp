...
std::vector<T> repeatVector(const std::vector<T>& vec,
  ...
  if (repeatNumber.size() == 1) {
    ...
    for (auto i = 0; i < repeatNumber[0]; ++i) {
      addOtherVec(ans, vec);
    }
...
std::vector<std::vector<T>> permuteVector(const std::vector<T>& vec,
  ...
  for (auto i = 0; i < numberOf; ++i) {
    ...
    auto numberOfCurrentRepeats =
        repeatVector(repeatFactorVec, sizeOfVectorVec);
  ...
  for (auto i = 0; i < colStop; ++i) {
    std::vector<T> tempVec;
    for (auto j = 0; j < rowStop; ++j) {
...
template <class T>
std::vector<T> getTargetsNotAtPositions(
    const std::vector<T>& vec, const std::vector<uint32_t>& positions) {
  std::vector<T> ans;
...
template <class T>
std::vector<uint32_t> getPositionsMultipleTargets(
    const std::vector<T>& vec, const std::vector<T>& targets) {
  std::vector<uint32_t> positions;
...
std::vector<std::vector<T>> findClumps(const std::vector<T>& positions,
                                       int windowSize, int appearsAtLeast,
                                       int extraLength = 0,
