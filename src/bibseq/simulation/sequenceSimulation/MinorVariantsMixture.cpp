...
size_t getUniquePos(bib::randomGenerator & gen,
...
void MinorVariantsMixture::addVariants(const std::vector<std::set<size_t>> & mutPositions){
	...
	for (const auto & positions : mutPositions) {
		...
		for (const auto pos : positions) {
			if(pos > len(*major_)){
				std::stringstream ss;
				ss << bib::bashCT::red << "Error in " << __PRETTY_FUNCTION__
...
void MinorVariantsMixture::createDisparatePairsMixture(uint32_t numberOfMinorStrains){
	...
	bib::iota<size_t>(allPossilblePositions,0);
...
void MinorVariantsMixture::createVaryingMinorVariantsMixture(uint32_t numberOfMinorVariants, double oneVariantPerIdDiff){
	...
	if(bib::in(maxDiffs, initialMuts)){
...
void MinorVariantsMixture::addVariants(const std::string & mutationsString, size_t mutationOffSet){
  //process mutations string
	...
	for (const auto & tok : mutToks) {
		std::string currentVariantName = major_->name_ + "_var" + estd::to_string(variants.size());
		...
		if (tok.find(":") != std::string::npos) {
			...
			for(const auto & smTok : specifcMutToks){
				...
				if (isIntStr(subMutToks[0]) && subMutToks[1].size() == 1
					...
					auto position = bib::lexical_cast<size_t>(subMutToks[0]);
					char base = subMutToks[1][0];
					if(position < len(*major_)){
						if(major_->seq_[position] != base){
							if(!bib::in(position, alreadyMutated )){
			...
			if (!bib::in(variant, variants) && variant != major_->seq_) {
				...
				std::stringstream ss;
				ss << bib::bashCT::red << "Error with creating variant: " << tok
			...
			if(mutationOffSet * 2 < len(*major_) ){
				...
				bib::iota<size_t>(allPositions, start);
				std::string variant = major_->seq_;
				while(bib::in(variant, variants) || variant == major_->seq_){
					variant = major_->seq_;
					std::vector<size_t> positions;
					bool alreadyContainsPositions = true;
					while(alreadyContainsPositions){
						positions = gen_.unifRandSelectionVec(allPositions, bib::lexical_cast<uint32_t>(tok),false);
						bool pass = true;
						for(auto & pos : positions){
							if(bib::in(pos, alreadyMutatedPosBase_)){
								pass = false;
								break;
							}
						}
...
void MinorVariantsMixture::createAbundancesForMixtures(
	...
	if (abundances.size() == 0) {
		std::stringstream ss;
		ss << bib::bashCT::red << "Error with processing mutations: "
				<< "mutation string is empty" << bib::bashCT::reset << std::endl;
		throw std::runtime_error { ss.str() };
	}
		...
		if (abTok.find(":") != std::string::npos) {
			...
			if (spAbToks.size() == numOfVars()) {
				...
				if (totalAbundance >= 100) {
					std::stringstream ss;
					ss << bib::bashCT::red << "Error with processing abundance: " << abTok
							<< ", requesting too much abundance, should be less than 100 total, current total: "
							<< totalAbundance << bib::bashCT::reset << std::endl;
					throw std::runtime_error { ss.str() };
				}
			...
			if (isDoubleStr(abTok)) {
				auto abund = bib::lexical_cast<double>(abTok);
				if (abund * numOfVars() < 100) {
					...
					ss << bib::bashCT::red << "Error with processing abundance: " << abTok
							...
							<< 99.0 / numOfVars() << bib::bashCT::reset << std::endl;
					throw std::runtime_error { ss.str() };
				}
			} else {
				std::stringstream ss;
				ss << bib::bashCT::red << "Error with processing abundance: " << abTok
						<< ", found non numeric character" << bib::bashCT::reset
						<< std::endl;
				throw std::runtime_error { ss.str() };
			}
		}
...
void MinorVariantsMixture::outputAbundanceFile(
	...
	for(const auto & mix : mixturesAbundances_){
		mixSorter[bib::lexical_cast<uint32_t>(mix.first.substr(mix.first.find(".") + 1))] = mix.first;
	}
