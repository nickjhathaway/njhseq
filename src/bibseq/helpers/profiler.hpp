...
VecStr profiler::compareToRefSingle(const std::vector<REF>& inputRefs,
  ...
  for (const auto& best : bestRead) {
    ...
    profile << best.seqBase_.name_ << "," << alignerObj.errors_.oneBaseIndel_
            << "," << alignerObj.errors_.twoBaseIndel_ << ","
            << alignerObj.errors_.largeBaseIndel_ << ","
            << alignerObj.errors_.hqMismatches_ +
                   alignerObj.errors_.lqMismatches_;
  ...
  for (const auto& input : inputClusters) {
    ...
    for (const auto& bestPos : bestRead) {
      ...
      alignerObj.profileAlignment(best, input, 2, false, false, true, false,
                                        weighHomopolyers);
      if(eventBased){
      	score = alignerObj.distances_.eventBasedIdentity_;
      } else {
      ...
      profileInfoFile << counter << "\t" << input.seqBase_.name_ << "\t"
                      ...
                      << "\t" << score << "\t"
                      << alignerObj.errors_.oneBaseIndel_ << "\t"
                      << alignerObj.errors_.twoBaseIndel_ << "\t"
                      << alignerObj.errors_.largeBaseIndel_ << "\t"
                      << alignerObj.errors_.lqMismatches_ << "\t"
                      << alignerObj.errors_.hqMismatches_ << std::endl;
    }
    counter++;
  }
}
   ...
   for (const auto& bestPos : bestRead) {
     ...
     if(verbose){
    	 out << delim << score << delim
           << alignerObj.errors_.oneBaseIndel_ << delim
           << alignerObj.errors_.twoBaseIndel_ << delim
           << alignerObj.errors_.largeBaseIndel_ << delim
           << alignerObj.errors_.lqMismatches_ << delim
           << alignerObj.errors_.hqMismatches_;
     }
   }
   return out.str();
