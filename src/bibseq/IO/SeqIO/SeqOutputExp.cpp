...
#include "SeqOutputExp.hpp"
#include <bibcpp/bashUtils.h>
#include "bibseq/IO/fileUtils.hpp"
namespace bibseq {
SeqOutputExp::SeqOutputExp(const SeqIOOptions & options) :
				ioOptions_(options) {}


bool SeqOutputExp::outOpen() const {
	return outOpen_;
}

void SeqOutputExp::openOut() {
	if(!outOpen_){
		std::stringstream ss;
...
void SeqOutputExp::closeOut() {
	primaryOut_ = nullptr;
	secondaryOut_ = nullptr;
	outOpen_ = false;
}

void SeqOutputExp::closeOutForReopening() {
	closeOut();
...
void SeqOutputExp::write(const seqInfo & read) {
	if (!outOpen_) {
...
void SeqOutputExp::writeNoCheck(const seqInfo & read) {
	switch (ioOptions_.outFormat_) {
	case SeqIOOptions::outFormats::FASTA:
...
void SeqOutputExp::openWrite(const seqInfo & read) {
	if (!outOpen_) {
		openOut();
	}
	writeNoCheck(read);
...
void SeqOutputExp::writeNoCheck(const PairedRead & seq) {
...
void SeqOutputExp::write(const PairedRead & read) {
	if (!outOpen_) {
...
void SeqOutputExp::openWrite(const PairedRead & read) {
	if (!outOpen_) {
		openOut();
	}
	writeNoCheck(read);
...
void SeqOutputExp::openWriteFlow(const sffObject & read) {
	if (!outOpen_) {
		openOut();
	}
...
void SeqOutputExp::writeNoCheckFlow(const sffObject & read) {
	read.outPutFlowValuesPryo(*primaryOut_);
}

void SeqOutputExp::openWrite(const std::string & line){
	if (!outOpen_) {
		openOut();
	}
...
void SeqOutputExp::writeNoCheck(const std::string & line){
	(*primaryOut_) << line << std::endl;
}

size_t SeqOutputExp::tellpPri(){
	return primaryOut_->tellp();
}

void SeqOutputExp::seekpPri(size_t pos){
	primaryOut_->seekp(pos);
}

size_t SeqOutputExp::tellpSec(){
	return secondaryOut_->tellp();
}
//Error in readObjectIOOpt, attempted to write when out files aren't open
void SeqOutputExp::seekpSec(size_t pos){
