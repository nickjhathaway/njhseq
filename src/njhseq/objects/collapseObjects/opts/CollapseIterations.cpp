...
CollapseIterations CollapseIterations::gen454ItDefaultPars(uint32_t stopCheck) {
	...
	std::vector<std::vector<double>> iters = {
		...
		{stopCheckDbl, 3, 2, 0, 0, 0, 0, 2},
		{stopCheckDbl, 3, 1, 0, 0, 0, 0, 3},
		...
		{stopCheckDbl, 3, 7, 5, .99, 0, 5, 3},
		{stopCheckDbl, 0, 1, 0, 0, 0, 0, 1},
		{stopCheckDbl, 0, 2, 0, 0, 0, 0, 1},
		{stopCheckDbl, 0, 2, 0, 0, 0, 0, 2},
		{stopCheckDbl, 0, 2, 0, 0, 0, 0, 3},
...
CollapseIterations CollapseIterations::genIlluminaDefaultAllLowMismatchesPars(uint32_t stopCheck) {
	...
	std::vector<std::vector<double>> iters = {
		...
		{stopCheckDbl, 3, 2, 0, 0, 0, 14, 2},
		{stopCheckDbl, 3, 2, 0, 0, 0, 14, 3},
		{stopCheckDbl, 0, 1, 0, 0, 0, 0, 1},
		{stopCheckDbl, 0, 1, 0, 0, 0, 0, 2},
		{stopCheckDbl, 0, 1, 0, 0, 0, 0, 3},
		{stopCheckDbl, 0, 2, 0, 0, 0, 0, 1},
...
CollapseIterations CollapseIterations::genIlluminaDefaultParsAllLowMismatchesCollapseHomopolymers(uint32_t stopCheck) {
	...
	std::vector<std::vector<double>> iters = {
		...
		{stopCheckDbl, 0, 1, .5, 0.5, 0, 0, 2},
		{stopCheckDbl, 0, 1, .5, 0.5, 0, 0, 3},
...
CollapseIterations CollapseIterations::genStrictDefaultParsWithHqs(
	...
	iters.push_back( { stopCheckDbl, 3, indels, indels, 0, 0, 1, 0 });
	for (uint32_t hq : iter::range(hqMismatches)) {
		iters.push_back(
				{ stopCheckDbl, 3, indels, indels, 0, hq + 1.0, hq + 1.0, 0 });
		iters.push_back(
				{ stopCheckDbl, 3, indels, indels, 0, hq + 1.0, hq + 1.0, 0 });
	}
	iters.push_back(noErrorsStopSize3);
	iters.push_back(noErrorsAll);
	iters.push_back( { stopCheckDbl, 0, indels, indels, 0, 0, 1, 0 });
	for (uint32_t hq : iter::range(hqMismatches)) {
		iters.push_back(
				{ stopCheckDbl, 0, indels, indels, 0, hq + 1.0, hq + 1.0, 0 });
		iters.push_back(
				{ stopCheckDbl, 0, indels, indels, 0, hq + 1.0, hq + 1.0, 0 });
	}
	iters.push_back(noErrorsAll);
