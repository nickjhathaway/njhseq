	...
	for (const auto & seq : seqs) {
		...
		for (const auto & col : outTab.columnNames_) {
			if ("name" == col) {
				...
				row.emplace_back(estd::to_string(getSeqBase(seq).getFastqQualString(SangerQualOffset)));
			} else {
...
template<typename T>
table seqsToMetaTable(const std::vector<T> & seqs, bool includeAnsiQual = false){
	...
	colNames.emplace_back("seq");
	if(includeAnsiQual) {
		colNames.emplace_back("qual");
	}
	std::set<std::string> fields(colNames.begin(), colNames.end());
	return seqsToMetaTable(seqs, fields);
	// table outTab(colNames);
	//
	// for (const auto & seq : seqs) {
	// 	VecStr row;
