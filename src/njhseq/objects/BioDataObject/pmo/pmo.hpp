...
public:
    ...
    uint32_t sequencing_info_id_{std::numeric_limits<uint32_t>::max()};
...
public:
    ...
    std::string collection_country_;
    std::string collection_date_;
    ...
    uint32_t project_id_{std::numeric_limits<uint32_t>::max()};
    ...
    std::vector<uint32_t> specimen_taxon_id_;
...
public:
    ...
    std::vector<BioinformaticsMethodInfo> bioinformatics_methods_info_;
    std::vector<BioinformaticsRunInfo> bioinformatics_run_info_;
    ...
    std::vector<ProjectInfo> project_info_;
    ...
    std::vector<SequencingInfo> sequencing_info_;
    std::vector<SpecimenInfo> specimen_info_;
    std::vector<TargetInfo> target_info_;
    std::vector<GenomeInfo> targeted_genomes_;
    std::map<std::string, nlohmann::json> extras_;
