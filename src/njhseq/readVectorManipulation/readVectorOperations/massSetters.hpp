...
void allSetCondensedSeq(std::vector<T>& reads) {
  njh::for_each(reads, [](T& read) { getRef(read).createCondensedSeq(); });
}
...
void allSetCondensedSeqCount(std::vector<T>& reads) {
  njh::for_each(reads, [](T& read) { getRef(read).setCondensedCounter(); });
}
...
void allUpdateName(std::vector<T>& reads) {
  njh::for_each(reads, [](T& read) { getRef(read).updateName(); });
}

template <class T>
void allSetFractionByTotalCount(std::vector<T>& seqs) {
  double count = getTotalReadCount(seqs);
  njh::for_each(seqs, [&](T& seq) { getSeqBase(seq).setFractionByCount(count); });
}

template <class T>
void allSetQualCheck(std::vector<T>& reads, int qualCheck) {
  njh::for_each(reads, [&](T& read) { getRef(read).setBaseCountOnQualCheck(qualCheck); });
}
